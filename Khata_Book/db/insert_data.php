<?php
include_once "fetch_userdetails.php";
include_once "connection.php";
function insert_transaction($id,$amount,$type,$source,$remark){
    ...
    try {
        ...
        $query1 = mysqli_prepare($conn, "INSERT INTO user_credits (UserID, amount,transaction_type, source,remark,updated_balance) VALUES (?, ?, ?, ?, ?, ?)");
        if (!$query1) {
            throw new Exception("Failed to prepare statement: " . mysqli_error($conn));
        }
    
        // Bind parameters for the first statement
        mysqli_stmt_bind_param($query1, "iisssi", $id, $amount, $type, $source, $remark, $updated_balance);
